 ...
 */
arm_status modified_arm_mat_inverse_f32(
...
arm_status mat_lu_decomposition(arm_matrix_instance_f32* src, arm_matrix_instance_f32* lt, arm_matrix_instance_f32* ut){
    ...
    rows = src->numRows;
    columns = src->numCols;
    if(!(rows == lt->numRows && rows == ut->numRows && columns == lt->numCols && columns == ut->numCols)){
    ...
    for(int i = 0; i < rows; i++){
        for(int j = 0; j < columns; j++){
            (*ut).pData[i*columns+j] = 0;
            if(i == j){
                (*lt).pData[i*columns+j] = 1;
            }
            else{
                (*lt).pData[i*columns+j] = 0;
            }
        }
    }

    for(int i = 0; i < columns; i++){
        (*ut).pData[i] = (*src).pData[i];
    }

    for(int j = 0; j < columns; j++) {
        for (int i = j; i < (rows-1); i++) {
            l = (*src).pData[(i + 1) * columns + j] / (*src).pData[j * columns + j];

            for(int z = 0; z < columns; z++){
                float32_t value = (*src).pData[(i + 1) * columns + z] - l * (*src).pData[j * columns + z];
                (*src).pData[(i + 1) * columns + z] = value;
            }

            (*lt).pData[(i + 1) * columns + j] = l;
        }
    }
